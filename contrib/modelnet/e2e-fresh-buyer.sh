...
SEEDER="${SEEDER_HOST_NAME}:${SEEDER_PORT:-29448}"
...
ssh -o BatchMode=yes "$FETCHER" \
  "export BTX_MODELD_DIRNAME=e2e-fresh-buyer; python3 /tmp/granite_second_process_retrieve.py --host '$SEEDER' --uri '$URI' --timeout ${WAN_TIMEOUT_S} --keep"
check_pids "$FETCHER" "${FETCHER_PROD_PIDS:-}"

...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_btx-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_btx-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_btx-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Wallet model notifications are asynchronous; under sanitizers the initial
    // transaction table population can lag behind test setup.
    QTRY_COMPARE_WITH_TIMEOUT(transactionTableModel->rowCount({}), 105, 30000);
...
void TestGUIWatchOnly(interfaces::Node& node, TestChain100Setup& test)
{
    ...
    auto find_psbt_dlg = []() -> PSBTOperationsDialog * {
        for (QWidget* widget : QApplication::topLevelWidgets()) {
    ...
    QTRY_VERIFY_WITH_TIMEOUT(find_psbt_dlg() != nullptr, 10000);
    auto* psbt_dlg = find_psbt_dlg();
    QVERIFY(psbt_dlg);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_btx-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);

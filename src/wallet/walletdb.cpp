...
const std::string PQMASTERSEEDCRYPT{"pqmasterseedcrypt"};
const std::string SHIELDEDSTATECRYPT{"shieldedstatecrypt"};
...
bool WalletBatch::WriteDescriptorPQPubKeyCache(PQAlgorithm algo, Span<const unsigned char> pubkey, const uint256& desc_id, uint32_t key_exp_index, uint32_t der_index)
{
    return WriteIC(
        std::make_pair(std::make_pair(DBKeys::WALLETDESCRIPTORPQCACHE, desc_id),
                       std::make_pair(std::make_pair(static_cast<uint8_t>(algo), key_exp_index), der_index)),
...
bool WalletBatch::WriteDescriptorCacheItems(const uint256& desc_id, const DescriptorCache& cache)
{
    ...
    for (PQAlgorithm algo : {PQAlgorithm::ML_DSA_44, PQAlgorithm::SLH_DSA_128S}) {
        for (const auto& key_exp_pair : cache.GetCachedDerivedPQPubKeys(algo)) {
            for (const auto& der_pair : key_exp_pair.second) {
                if (!WriteDescriptorPQPubKeyCache(algo, der_pair.second, desc_id, key_exp_pair.first, der_pair.first)) {
                    return false;
                }
            }
        }
...
bool WalletBatch::WritePQMasterSeed(const std::vector<unsigned char>& seed)
{
    return WriteIC(DBKeys::PQMASTERSEED, seed);
}

bool WalletBatch::ReadPQMasterSeed(std::vector<unsigned char>& seed)
{
    return m_batch->Read(DBKeys::PQMASTERSEED, seed);
}

bool WalletBatch::ErasePQMasterSeed()
{
    return EraseIC(DBKeys::PQMASTERSEED);
}

bool WalletBatch::WriteCryptedPQMasterSeed(const uint256& iv, const std::vector<unsigned char>& seed)
{
    return WriteIC(DBKeys::PQMASTERSEEDCRYPT, std::make_pair(iv, seed));
}

bool WalletBatch::ReadCryptedPQMasterSeed(uint256& iv, std::vector<unsigned char>& seed)
{
    std::pair<uint256, std::vector<unsigned char>> payload;
    if (!m_batch->Read(DBKeys::PQMASTERSEEDCRYPT, payload)) {
        return false;
    }
    iv = payload.first;
...
bool WalletBatch::EraseCryptedPQMasterSeed()
{
    return EraseIC(DBKeys::PQMASTERSEEDCRYPT);
}

bool WalletBatch::WritePQDescriptorSeed(const uint256& desc_id, const std::vector<unsigned char>& seed)
{
    return WriteIC(std::make_pair(DBKeys::WALLETDESCRIPTORPQSEED, desc_id), seed);
}

bool WalletBatch::ReadPQDescriptorSeed(const uint256& desc_id, std::vector<unsigned char>& seed)
{
...
{
    return WriteIC(DBKeys::SHIELDEDSTATE, state);
}

bool WalletBatch::ReadShieldedState(std::vector<unsigned char>& state)
{
    return m_batch->Read(DBKeys::SHIELDEDSTATE, state);
}

bool WalletBatch::EraseShieldedState()
{
    return EraseIC(DBKeys::SHIELDEDSTATE);
}

bool WalletBatch::WriteCryptedShieldedState(const uint256& iv, const std::vector<unsigned char>& state)
{
    return WriteIC(DBKeys::SHIELDEDSTATECRYPT, std::make_pair(iv, state));
}

bool WalletBatch::ReadCryptedShieldedState(uint256& iv, std::vector<unsigned char>& state)
{
    std::pair<uint256, std::vector<unsigned char>> payload;
    if (!m_batch->Read(DBKeys::SHIELDEDSTATECRYPT, payload)) {
        return false;
    }
    iv = payload.first;
    state = std::move(payload.second);
    return true;
}

bool WalletBatch::EraseCryptedShieldedState()
{
    return EraseIC(DBKeys::SHIELDEDSTATECRYPT);
}

bool WalletBatch::WriteShieldedCommitments(const std::vector<uint256>& commitments)
{
    return WriteIC(DBKeys::SHIELDEDCOMMITMENTS, commitments);
}

bool WalletBatch::ReadShieldedCommitments(std::vector<uint256>& commitments)
{
    return m_batch->Read(DBKeys::SHIELDEDCOMMITMENTS, commitments);
}
...
static DBErrors LoadDescriptorWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    LoadResult desc_res = LoadRecords(pwallet, batch, DBKeys::WALLETDESCRIPTOR,
        ...
        // Get PQ pubkey cache for this descriptor
        ...
        LoadResult pq_cache_res = LoadRecords(pwallet, batch, DBKeys::WALLETDESCRIPTORPQCACHE, prefix,
            ...
            if (algo_u8 != static_cast<uint8_t>(PQAlgorithm::ML_DSA_44) && algo_u8 != static_cast<uint8_t>(PQAlgorithm::SLH_DSA_128S)) {
                err = "Error reading wallet database: unknown PQAlgorithm in descriptor cache";
                return DBErrors::CORRUPT;
            }
            const size_t expected_size = (algo_u8 == static_cast<uint8_t>(PQAlgorithm::ML_DSA_44)) ? MLDSA44_PUBKEY_SIZE : SLHDSA128S_PUBKEY_SIZE;
            const uint64_t serialized_size = ReadCompactSize(value);
            if (serialized_size != expected_size) {
                err = "Error reading wallet database: invalid PQ pubkey size in descriptor cache";
                return DBErrors::CORRUPT;
            }
            std::vector<unsigned char> pq_pubkey(expected_size);
            value.read(AsWritableBytes(Span{pq_pubkey}));
            cache.CacheDerivedPQPubKey(static_cast<PQAlgorithm>(algo_u8), key_exp_index, der_index, pq_pubkey);
            return DBErrors::LOAD_OK;
        });

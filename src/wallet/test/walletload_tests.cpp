...
BOOST_FIXTURE_TEST_CASE(wallet_load_descriptor_pq_cache_invalid_pubkey_size, TestingSetup)
{
    std::unique_ptr<WalletDatabase> database = CreateMockableWalletDatabase();
    uint256 desc_id;
    ...
    {
        WalletBatch batch(*database, false);
        std::vector<unsigned char> fixed_mldsa(MLDSA44_PUBKEY_SIZE, 0x42);
        std::vector<unsigned char> fixed_slh(SLHDSA128S_PUBKEY_SIZE, 0x24);
        const std::string desc_str = AddChecksum("mr(" + HexStr(fixed_mldsa) + ",pk_slh(" + HexStr(fixed_slh) + "))");

        FlatSigningProvider keys;
        std::string error;
        auto descs = Parse(desc_str, keys, error, /*require_checksum=*/true);
        BOOST_REQUIRE_MESSAGE(!descs.empty(), "descriptor parse failed: " + error);
        BOOST_REQUIRE_EQUAL(descs.size(), 1U);

        WalletDescriptor wallet_descriptor(std::move(descs.at(0)), /*creation_time=*/0, /*range_start=*/0, /*range_end=*/0, /*next_index=*/0);
        desc_id = wallet_descriptor.id;
        BOOST_CHECK(batch.WriteDescriptor(desc_id, wallet_descriptor));

        // Corrupt PQ pubkey cache entry: wrong pubkey size should fail wallet load as CORRUPT.
        std::vector<unsigned char> bad_pubkey(7, 0x99);
        BOOST_CHECK(batch.WriteDescriptorPQPubKeyCache(PQAlgorithm::ML_DSA_44, bad_pubkey, desc_id, /*key_exp_index=*/0, /*der_index=*/0));
    }

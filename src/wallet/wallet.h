...
//! -fallbackfee default
//! BTX: non-zero so that wallets work on young chains where estimateSmartFee
//! has insufficient data.  1000 sat/kvB = 1 sat/vB, same as DEFAULT_TRANSACTION_MINFEE.
...
//! BTX uses WITNESS_SCALE_FACTOR=1, so track the maximum serialized nested-P2WPKH input size.
static constexpr size_t DUMMY_NESTED_P2WPKH_INPUT_SIZE = WITNESS_SCALE_FACTOR == 1 ? 172 : 91;
...
private:
     ...
     * NOTE: One exception to this is that the abandoned state is cleared under the
    ...
    //! Update the in-memory wallet scan position without writing the database.
...
public:
    ...
    /** Shielded wallet state manager (initialized when wallet chain is attached). */
    std::shared_ptr<CShieldedWallet> m_shielded_wallet;
    ...
    /** Abandon shielded transactions that are unconfirmed and no longer in the
     *  actual mempool (e.g. rejected with bad-shielded-anchor).  This does not
     *  require the shielded wallet to be initialised or the wallet to be
     *  unlocked — it only inspects wallet tx state and the live mempool. */
    void AbandonStuckShieldingTransactions() EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    ScanResult ScanForWalletTransactions(const uint256& start_block, int start_height, std::optional<int> max_height, const WalletRescanReserver& reserver, bool fUpdate, const bool save_progress, bool scan_shielded = false);
    ...
    bool SubmitTxMemoryPoolAndRelay(CWalletTx& wtx,
                                    ...
                                    bool relay,
                                    bool bypass_maxtxfee = false) const
    ...
    /** Set the last processed block and persist its locator. */
    void SetLastBlockProcessed(int block_height, uint256 block_hash) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    /** Persist the locator matching the wallet's current scan position. */
    ...
    //! Disconnect chain notifications and wait for pending callbacks to finish.
...
public:
    ...
    bool reserve(bool with_passphrase = false)
    {
        ...
        // Drop an abort request left over from an earlier reservation. An
        // abort arriving after this point must apply to this rescan.
...
//! Determine and validate the filesystem path used by a wallet name.

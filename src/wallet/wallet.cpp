...
int64_t DepthFromHeights(const int tip_height, const int tx_height)
{
    return static_cast<int64_t>(tip_height) - static_cast<int64_t>(tx_height) + 1;
}

bool PSBTHasExistingSignatures(const PartiallySignedTransaction& psbtx)
{
    return std::any_of(psbtx.inputs.begin(), psbtx.inputs.end(), [](const PSBTInput& input) {
        return !input.partial_sigs.empty() ||
...
std::shared_ptr<CWallet> RestoreWallet(WalletContext& context, const fs::path& backup_file, const std::string& wallet_name, std::optional<bool> load_on_start, DatabaseStatus& status, bilingual_str& error, std::vector<bilingual_str>& warnings, bool load_after_restore)
{
    ...
    try {
        ...
        // Wallet directories are allowed to exist, but must not contain wallet.dat.
...
{
    // The chain tip may be ahead of asynchronous wallet notifications. The
    // wallet persists its own scan position after block connect/disconnect.
    (void)role;
    (void)loc;
}
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose, bool rescanning_old_block)
{
    ...
    if (!fInsertedNew)
    {
        ...
        // NOTE: Store all versions of the transaction, instead of just one.
...
bool CWallet::AbandonTransaction(CWalletTx& tx)
{
    ...
    if (tx.isConfirmed() || tx.InMempool() || GetTxDepthInMainChain(tx) != 0) {
        return false;
    }

    auto try_updating_state = [](CWalletTx& wtx) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet) {
        // Skip descendants that are currently confirmed/in-mempool.
...
void CWallet::MarkConflicted(const uint256& hashBlock, int conflicting_height, const uint256& hashTx)
{
    ...
    const int64_t conflict_confirms_64 = -DepthFromHeights(m_last_block_processed_height, conflicting_height);
    const int conflictconfirms = SaturatingIntFromInt64(conflict_confirms_64);
...
void CWallet::transactionAddedToMempool(const CTransactionRef& tx) {
    ...
    if (m_shielded_wallet && tx->HasShieldedBundle()) {
        LOCK(m_shielded_wallet->cs_shielded);
        m_shielded_wallet->TransactionAddedToMempool(*tx);
    }
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason) {
    ...
    // Clean up shielded wallet mempool tracking for evicted transactions.
    if (m_shielded_wallet && tx->HasShieldedBundle()) {
        LOCK(m_shielded_wallet->cs_shielded);
        m_shielded_wallet->TransactionRemovedFromMempool(*tx);
    }
...
void CWallet::AbandonStuckShieldingTransactions()
{
    ...
    for (auto& [txid, wtx] : mapWallet) {
        ...
        // Use chain().isInMempool() rather than wtx.InMempool() because the
        // wallet tx state may be stale: a tx saved as TxStateInMempool in a
        // prior session retains that state on reload even though it is no
        // longer in the actual mempool.
        if (chain().isInMempool(wtx.GetHash())) continue;

        WalletLogPrintf("AbandonStuckShieldingTransactions: abandoning stuck shielding tx %s\n",
                        txid.GetHex());
        AbandonTransaction(wtx);
        ++abandoned_count;
    }
    if (abandoned_count > 0) {
        WalletLogPrintf("AbandonStuckShieldingTransactions: abandoned %d stuck shielding transaction(s)\n", abandoned_count);
    }
}

void CWallet::MaybeAutoShieldCoinbase()
{
    AssertLockHeld(cs_wallet);
    if (!m_shielded_wallet) return;
    if (IsLocked()) return;
    ...
    try {
        // Clean up stuck shielding transactions before looking for new outputs
        // to shield. This reclaims coinbase outputs that were locked by
        // previously-failed auto-shield attempts.
        AbandonStuckShieldingTransactions();

        // Collect mature, unspent coinbase outputs owned by this wallet.
        std::vector<COutPoint> mature_coinbase_utxos;
        CAmount total_in{0};
        static constexpr int AUTO_SHIELD_BATCH_LIMIT{50};
        std::map<CScript, bool> script_signable_cache;
        bool saw_unsignable_candidate{false};
        const auto can_sign_script = [&](const uint256& txid, const CWalletTx& wtx, uint32_t n) -> bool {
            const CScript& script_pub_key = wtx.tx->vout[n].scriptPubKey;
            const auto cache_it = script_signable_cache.find(script_pub_key);
            if (cache_it != script_signable_cache.end()) {
                return cache_it->second;
            }

            bool can_sign = false;
            const CAmount input_value = wtx.tx->vout[n].nValue;
            if (MoneyRange(input_value) && input_value > 0) {
                CMutableTransaction probe_tx;
                probe_tx.version = CTransaction::CURRENT_VERSION;
                const COutPoint probe_outpoint{Txid::FromUint256(txid), n};
                probe_tx.vin.emplace_back(probe_outpoint);

                const CAmount probe_fee{1000};
                if (input_value > probe_fee) {
                    probe_tx.vout.emplace_back(input_value - probe_fee, script_pub_key);
                } else {
                    probe_tx.vout.emplace_back(input_value, script_pub_key);
                }

                std::map<COutPoint, Coin> coins;
                const int prev_height = wtx.state<TxStateConfirmed>() ? wtx.state<TxStateConfirmed>()->confirmed_block_height : 0;
                coins.emplace(probe_outpoint, Coin(wtx.tx->vout[n], prev_height, wtx.IsCoinBase()));

                std::map<int, bilingual_str> input_errors;
                can_sign = SignTransaction(probe_tx, coins, SIGHASH_DEFAULT, input_errors);
                if (!can_sign && !input_errors.empty()) {
                    const auto& [input_index, err] = *input_errors.begin();
                    WalletLogPrintf("AutoShieldCoinbase: unsignable script probe input=%d err=%s\n",
                                    input_index,
                                    err.original);
                }
            }

            script_signable_cache.emplace(script_pub_key, can_sign);
            return can_sign;
        };

        for (const auto& [txid, wtx] : mapWallet) {
            if (!wtx.IsCoinBase()) continue;
            if (GetTxBlocksToMaturity(wtx) > 0) continue;
            for (uint32_t n = 0; n < wtx.tx->vout.size(); ++n) {
                if (!(IsMine(wtx.tx->vout[n]) & ISMINE_SPENDABLE)) continue;
                if (!can_sign_script(txid, wtx, n)) {
                    saw_unsignable_candidate = true;
                    continue;
                }
                const COutPoint outpoint{Txid::FromUint256(txid), n};
                if (IsSpent(outpoint)) continue;
        ...
        if (mature_coinbase_utxos.empty()) {
            if (saw_unsignable_candidate) {
                m_autoshield_retry_after_height = m_last_block_processed_height + AUTO_SHIELD_RETRY_BLOCKS;
                WalletLogPrintf("AutoShieldCoinbase: no signable mature coinbase outputs, next retry at height %d\n",
                                m_autoshield_retry_after_height);
            }
            return;
        }
        ...
        for (int attempt = 0; attempt < 3; ++attempt) {
            {
                LOCK(m_shielded_wallet->cs_shielded);
                mtx = m_shielded_wallet->ShieldFunds(mature_coinbase_utxos, fee);
            }
            if (!mtx.has_value()) break;

            const CTransaction candidate_tx{*mtx};
            const CAmount required_fee = RequiredShieldedMempoolFee(*this, candidate_tx);
            if (fee >= required_fee) break;

            WalletLogPrintf("AutoShieldCoinbase: fee %s below required relay floor %s, rebuilding\n",
                            FormatMoney(fee),
        ...
        if (!mtx.has_value()) {
            m_autoshield_retry_after_height = m_last_block_processed_height + AUTO_SHIELD_RETRY_BLOCKS;
            WalletLogPrintf("AutoShieldCoinbase: failed to build shielding transaction, next retry at height %d\n",
                            m_autoshield_retry_after_height);
            return;
        }

        CTransactionRef tx = MakeTransactionRef(std::move(*mtx));
        CommitTransaction(tx, /*mapValue=*/{}, /*orderForm=*/{}, /*bypass_maxtxfee=*/true);

        if (GetBroadcastTransactions()) {
            if (chain().shutdownRequested()) {
                WalletLogPrintf("AutoShieldCoinbase: shutdown requested before broadcast, abandoning tx %s\n",
                                tx->GetHash().GetHex());
                AbandonTransaction(tx->GetHash());
                m_autoshield_retry_after_height = m_last_block_processed_height + 1;
                return;
            }
            std::string err_string;
            chain().broadcastTransaction(tx, DEFAULT_TRANSACTION_MAXFEE, /*relay=*/true, err_string);
            if (!err_string.empty()) {
                WalletLogPrintf("AutoShieldCoinbase: broadcast failed: %s, abandoning tx %s\n",
                                err_string, tx->GetHash().GetHex());
                AbandonTransaction(tx->GetHash());
                m_autoshield_retry_after_height = m_last_block_processed_height + AUTO_SHIELD_RETRY_BLOCKS;
                return;
            }
        ...
        m_autoshield_retry_after_height = m_last_block_processed_height + 1;
        WalletLogPrintf("AutoShieldCoinbase: shielded %d coinbase outputs (%s) in tx %s\n",
                        mature_coinbase_utxos.size(),
                        FormatMoney(total_in),
                        tx->GetHash().GetHex());
    } catch (const std::exception& e) {
        m_autoshield_retry_after_height = m_last_block_processed_height + AUTO_SHIELD_RETRY_BLOCKS;
        WalletLogPrintf("AutoShieldCoinbase: exception while shielding: %s (next retry at height %d)\n",
                        e.what(),
                        m_autoshield_retry_after_height);
    } catch (...) {
        m_autoshield_retry_after_height = m_last_block_processed_height + AUTO_SHIELD_RETRY_BLOCKS;
        WalletLogPrintf("AutoShieldCoinbase: unknown exception while shielding (next retry at height %d)\n",
                        m_autoshield_retry_after_height);
    }
}
...
void CWallet::blockDisconnected(const interfaces::BlockInfo& block)
{
    assert(block.data);
    {
        ...
        SetLastBlockProcessedInMem(block.height - 1, *Assert(block.prev_hash));
        ...
        // Persist only after the disconnected block has been fully undone.
        WriteBestBlock();
        GetDatabase().IncrementUpdateCounter();
    }
...
int64_t CWallet::RescanFromTime(int64_t startTime, const WalletRescanReserver& reserver, bool update)
{
    ...
    const int rescan_blocks = start ? SaturatingIntFromInt64(static_cast<int64_t>(WITH_LOCK(cs_wallet, return GetLastBlockHeight())) - start_height + 1) : 0;
    WalletLogPrintf("%s: Rescanning last %i blocks\n", __func__, rescan_blocks);

    if (start) {
        // NOTE: this should take into account failure by ScanResult::USER_ABORT
 ...
 * transactions for.
 */
CWallet::ScanResult CWallet::ScanForWalletTransactions(const uint256& start_block, int start_height, std::optional<int> max_height, const WalletRescanReserver& reserver, bool fUpdate, const bool save_progress, bool scan_shielded)
{
    ...
    // BIP158 block filters only cover transparent scriptPubKeys.  When a
    // shielded rescan is requested every block must be fetched so that
    // ScanBlock can trial-decrypt shielded outputs, so skip the fast filter.
    if (!scan_shielded && !IsLegacy() && chain().hasBlockFilterIndex(BlockFilterType::BASIC)) fast_rescan_filter = std::make_unique<FastWalletRescanFilter>(*this);
    ...
    while (!fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (fetch_block) {
            ...
            if (!block.IsNull()) {
                ...
                if (scan_shielded && m_shielded_wallet) {
                    LOCK(m_shielded_wallet->cs_shielded);
                    m_shielded_wallet->ScanBlock(block, block_height);
                }
                                         ...
                                         bool bypass_maxtxfee) const
{
    ...
    const CAmount max_tx_fee = bypass_maxtxfee ? 0 : m_default_max_tx_fee;
    bool ret = chain().broadcastTransaction(wtx.tx, max_tx_fee, relay, err_string);
...
//
// NOTE: Ideally, we'd only resend transactions that we think should have been
...
void CWallet::ResubmitWalletTransactions(bool relay, bool force)
{
    ...
    { // cs_wallet scope
        ...
        for (auto& [txid, wtx] : mapWallet) {
            ...
            // Skip shielded-bundle transactions on the forced startup path.
            // The consensus shielded tree may not have caught up to the chain
            // tip yet, so anchor validation would reject them.  For encrypted
            // wallets the wallet is still locked, so they can never succeed at
            // this point.  These transactions will be handled later by
            // MaybeAutoShieldCoinbase once the chain is synced and the wallet
            // is unlocked.
            if (force && wtx.tx->HasShieldedBundle()) continue;
...
bool CWallet::SignTransaction(CMutableTransaction& tx, std::optional<PQAlgorithm> preferred_pq_signing_algo) const
{
    AssertLockHeld(cs_wallet);
...
bool CWallet::SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, std::optional<CAmount>* inputs_amount_sum, std::optional<PQAlgorithm> preferred_pq_signing_algo) const
{
    ...
    // Cross-descriptor PQ key merge: for multisig P2MR descriptors, the leaf
    // scripts and p2mr_spends live in the multisig descriptor's spk_man, but
    // PQ private keys may only be derivable from separate solo descriptors.
    // Build a combined provider by merging p2mr_spends from the descriptor
    // that owns each coin's script with PQ keys from ALL descriptors.
    {
        FlatSigningProvider merged;
        bool have_p2mr_spends{false};
        for (const auto& coin_pair : coins) {
            for (ScriptPubKeyMan* spk_man : GetAllScriptPubKeyMans()) {
                auto* desc_spk_man = dynamic_cast<DescriptorScriptPubKeyMan*>(spk_man);
                if (!desc_spk_man) continue;
                auto coin_keys = desc_spk_man->GetSigningProvider(coin_pair.second.out.scriptPubKey, true);
                if (coin_keys) {
                    if (!coin_keys->p2mr_spends.empty()) have_p2mr_spends = true;
                    merged.Merge(std::move(*coin_keys));
                    break;
                }
            }
        }
        if (have_p2mr_spends && merged.pq_keys.empty()) {
            // Extract PQ pubkeys from the leaf scripts in p2mr_spends, then
            // look each one up across all descriptors to find matching private keys.
            std::set<std::vector<unsigned char>> pq_pubkeys_needed;
            for (const auto& [output, spenddata] : merged.p2mr_spends) {
                for (const auto& [leaf_script, controls] : spenddata.scripts) {
                    // Parse PQ pubkeys from the leaf script: look for known PQ key sizes
                    // embedded as push data in the script.
                    const CScript script_obj(leaf_script.begin(), leaf_script.end());
                    CScript::const_iterator pc = script_obj.begin();
                    opcodetype opcode;
                    std::vector<unsigned char> push_data;
                    while (script_obj.GetOp(pc, opcode, push_data)) {
                        if (push_data.size() == MLDSA44_PUBKEY_SIZE || push_data.size() == SLHDSA128S_PUBKEY_SIZE) {
                            pq_pubkeys_needed.insert(push_data);
                        }
                    }
                }
            }
            for (const auto& pq_pub : pq_pubkeys_needed) {
                for (ScriptPubKeyMan* spk_man : GetAllScriptPubKeyMans()) {
                    auto* desc_spk_man = dynamic_cast<DescriptorScriptPubKeyMan*>(spk_man);
                    if (!desc_spk_man) continue;
                    auto pq_keys = desc_spk_man->GetSigningProvider(Span<const unsigned char>{pq_pub});
                    if (pq_keys && !pq_keys->pq_keys.empty()) {
                        merged.Merge(std::move(*pq_keys));
                        break;
                    }
                }
            }
...
std::optional<PSBTError> CWallet::FillPSBT(PartiallySignedTransaction& psbtx, bool& complete, int sighash_type, bool sign, bool bip32derivs, size_t * n_signed, bool finalize) const
{
    ...
    std::set<ScriptPubKeyMan*> spk_mans;
    {
        ...
        // Get all of the previous transactions from wallet state.
        ...
        // Snapshot script managers while wallet state is locked.
    ...
    p2mr_timelocked_inputs.reserve(psbtx.inputs.size());
    for (unsigned int i = 0; i < psbtx.inputs.size(); ++i) {
        const PSBTInput& input = psbtx.inputs.at(i);
        if (!input.m_p2mr_leaf_script.empty()) {
            p2mr_timelocked_inputs.push_back({i, input.m_p2mr_leaf_script});
        }
    }
    ...
    // Fill in information from ScriptPubKeyMans outside cs_wallet to avoid
...
{
    // DescriptorScriptPubKeyMan::SignMessage may query wallet lock state, and
    // the wallet-wide fallback below reads all script managers and keys.
    LOCK(cs_wallet);
    ...
    for (const auto& spk_man_pair : m_spk_managers) {
        if (spk_man_pair.second->CanProvide(script_pub_key, sigdata)) {
            const SigningResult result{spk_man_pair.second->SignMessage(format, message, address, str_sig)};
            if (result == SigningResult::OK ||
                format == MessageSignatureFormat::LEGACY ||
                !std::holds_alternative<WitnessV2P2MR>(address)) {
                return result;
            }
            // A watch-only P2MR descriptor can provide the challenge script
            // but not its key. Fall through to the wallet-wide merge below.
        }
    }

    // A fixed P2MR descriptor may provide the exact message-challenge script
    // while its PQ private key is held by a different descriptor in the same
    // wallet. This is common for protocol-specific, single-key attestations
    // derived from a key that normally lives below a ranged wallet descriptor.
    // Reuse the wallet-wide transaction signer so its cross-descriptor PQ key
    // merge can assemble both halves of the signing provider.
    if (format != MessageSignatureFormat::LEGACY &&
        std::holds_alternative<WitnessV2P2MR>(address)) {
        MessageVerificationResult result;
        const auto txs{BIP322Txs::Create(address, message, result)};
        if (txs) {
            CMutableTransaction to_sign{txs->m_to_sign};
            std::map<COutPoint, Coin> coins;
            coins.emplace(to_sign.vin[0].prevout, Coin(txs->m_to_spend.vout[0], 1, false));
...
util::Result<fs::path> GetWalletPath(const std::string& name)
{
    ...
    if (!name_path.empty() && (*name_path.begin() == fs::PathFromString(".") || *name_path.begin() == fs::PathFromString(".."))) {
        return util::Error{Untranslated("Wallet name given as a relative path cannot begin with ./ or ../; use an absolute path for wallets outside the wallet directory")};
    }
    ...
    if (!(path_type == fs::file_type::not_found || path_type == fs::file_type::directory ||
          ...
          // Windows cross builds may report symlinks to files as regular files.
...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    // NOTE: Can't use std::make_shared because we need a custom deleter but
    ...
    if (!args.GetArg("-addresstype", "").empty()) {
        ...
        if (!parsed) {
            ...
            return nullptr;
        } else if (parsed.value() != OutputType::P2MR) {
            error = strprintf(_("Unsupported address type '%s' (only p2mr is supported)"), args.GetArg("-addresstype", ""));
            return nullptr;
        }
    ...
    if (!args.GetArg("-changetype", "").empty()) {
        ...
        if (!parsed) {
            ...
            error = strprintf(_("Unsupported change type '%s' (only p2mr is supported)"), args.GetArg("-changetype", ""));
            return nullptr;
        }
...
bool CWallet::AttachChain(const std::shared_ptr<CWallet>& walletInstance, interfaces::Chain& chain, const bool rescan_required, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (!walletInstance->m_shielded_wallet) {
        walletInstance->m_shielded_wallet = std::make_shared<CShieldedWallet>(*walletInstance);
    }
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart btxd with -walletcrosschain to override.");
                return false;
            }
        }
    }
    ...
    // Update transaction states only after chain notifications are attached,
    // so blocks connected during wallet loading cannot be missed. Reverse
    // order preserves recursive abandonment of transaction descendants.
    ...
    if (tip_height && *tip_height != rescan_height)
    {
...

        {
            ...
            const ScanResult scan_result = walletInstance->ScanForWalletTransactions(chain.getBlockHash(rescan_height), rescan_height, /*max_height=*/{}, reserver, /*fUpdate=*/true, /*save_progress=*/true);
            if (ScanResult::SUCCESS != scan_result.status) {
                error = _("Failed to rescan the wallet during initialization");
                return false;
            }
            walletInstance->m_attaching_chain = false;
            walletInstance->SetLastBlockProcessed(*Assert(scan_result.last_scanned_height), scan_result.last_scanned_block);
        }
    }
    walletInstance->m_attaching_chain = false;
...
void CWallet::postInitProcess()
{
    ...
    // Abandon shielding transactions left over from a previous session that
    // are unconfirmed and no longer in the mempool.  ResubmitWalletTransactions
    // above already skips shielded-bundle txs (they would fail anchor
    // validation before the chain catches up), so these are genuinely stuck
    // from a prior run.  Doing this now frees the locked coinbase outputs
    // immediately rather than waiting for the next block to connect.
    WITH_LOCK(cs_wallet, AbandonStuckShieldingTransactions());
...
{
    if (m_chain) WITH_LOCK(cs_wallet, WriteBestBlock());

    // Flush shielded wallet state to the database before backup so the
    // backup file contains the latest note index, witnesses, and Merkle
    // tree.  Without this, a raw database backup may contain stale
    // shielded state if PersistState was deferred or only held in memory.
    if (m_shielded_wallet) {
        LOCK(m_shielded_wallet->cs_shielded);
        if (!m_shielded_wallet->PersistState()) {
            LogPrintf("CWallet::BackupWallet: warning — failed to flush shielded state before backup\n");
        }
    }
...
{
    AssertLockHeld(cs_wallet);
    // During wallet load (before chain attach), the wallet may classify tx state
    // while last-processed chain height is still unset.
    if (m_last_block_processed_height < 0) {
        return 0;
    }
    const int last_height = m_last_block_processed_height;
    if (auto* conf = wtx.state<TxStateConfirmed>()) {
        assert(conf->confirmed_block_height >= 0);
        return SaturatingIntFromInt64(DepthFromHeights(last_height, conf->confirmed_block_height));
    } else if (auto* conf = wtx.state<TxStateBlockConflicted>()) {
        assert(conf->conflicting_block_height >= 0);
        return SaturatingIntFromInt64(-DepthFromHeights(last_height, conf->conflicting_block_height));
    } else {
        return 0;
    }
}
...
DescriptorScriptPubKeyMan& CWallet::SetupPQDescriptorScriptPubKeyMan(WalletBatch& batch, Span<const unsigned char> pq_seed, bool internal)
{
    AssertLockHeld(cs_wallet);
    auto spk_manager = std::unique_ptr<DescriptorScriptPubKeyMan>(new DescriptorScriptPubKeyMan(*this, m_keypool_size));
    if (IsCrypted()) {
...
void CWallet::SetupOwnPQDescriptorScriptPubKeyMans(WalletBatch& batch)
{
    ...
    std::array<DescriptorScriptPubKeyMan*, 2> existing_p2mr{nullptr, nullptr};
    bool needs_new_p2mr{false};
    for (const bool internal : {false, true}) {
        if (auto* existing_spkm = dynamic_cast<DescriptorScriptPubKeyMan*>(GetScriptPubKeyMan(OutputType::P2MR, internal))) {
            existing_p2mr[internal ? 1 : 0] = existing_spkm;
            AddActiveScriptPubKeyManWithDb(batch, existing_spkm->GetID(), OutputType::P2MR, internal);
        } else {
            needs_new_p2mr = true;
        }
    }
    ...
    constexpr std::string_view kPQMasterSeedPurpose{"pqmasterseed"};
    auto persist_encrypted_seed = [&](const std::vector<unsigned char>& seed) {
        if (!IsCrypted()) return;
        if (IsLocked()) {
            throw std::runtime_error(std::string(__func__) + ": Wallet is locked, cannot persist encrypted PQ master seed");
        }
        uint256 iv = GetRandHash();
        std::vector<unsigned char> encrypted_seed;
        const bool encrypted = WithEncryptionKey([&](const CKeyingMaterial& encryption_key) {
            return EncryptAuthenticatedSecret(encryption_key,
                                              std::span<const unsigned char>{seed.data(), seed.size()},
                                              iv,
                                              encrypted_seed,
                                              kPQMasterSeedPurpose);
        });
        if (!encrypted) {
            throw std::runtime_error(std::string(__func__) + ": encrypting PQ master seed failed");
        }
        if (!batch.WriteCryptedPQMasterSeed(iv, encrypted_seed)) {
            throw std::runtime_error(std::string(__func__) + ": writing encrypted PQ master seed failed");
        }
        if (!batch.ErasePQMasterSeed()) {
            WalletLogPrintf("%s: failed to erase plaintext PQ master seed after encrypted migration\n", __func__);
        }
    };

    // Reuse existing PQ master seed when present so shielded key derivation
    // remains stable across wallet encryption and descriptor manager refreshes.
    std::vector<unsigned char> seed_vec;
    uint256 iv;
    std::vector<unsigned char> encrypted_seed;
    if (batch.ReadCryptedPQMasterSeed(iv, encrypted_seed)) {
        if (encrypted_seed.empty()) {
            throw std::runtime_error(std::string(__func__) + ": stored encrypted PQ master seed is empty");
        }
        if (!IsCrypted()) {
            throw std::runtime_error(std::string(__func__) + ": encrypted PQ master seed present in unencrypted wallet");
        }
        if (IsLocked()) {
            throw std::runtime_error(std::string(__func__) + ": Wallet is locked, cannot load encrypted PQ master seed");
        }
        std::vector<unsigned char> decrypted_seed;
        bool was_authenticated{false};
        const bool decrypted = WithEncryptionKey([&](const CKeyingMaterial& encryption_key) {
        ...
        if (!decrypted || decrypted_seed.size() != 32) {
            if (!decrypted_seed.empty()) {
                memory_cleanse(decrypted_seed.data(), decrypted_seed.size());
            }
            throw std::runtime_error(std::string(__func__) + ": encrypted PQ master seed is unreadable or corrupted");
        }
        seed_vec.assign(decrypted_seed.begin(), decrypted_seed.end());
        memory_cleanse(decrypted_seed.data(), decrypted_seed.size());
        if (!was_authenticated) {
            persist_encrypted_seed(seed_vec);
        }
    } else if (batch.ReadPQMasterSeed(seed_vec)) {
        if (seed_vec.size() != 32) {
            throw std::runtime_error(std::string(__func__) + ": stored PQ master seed has invalid length");
        }
        if (IsCrypted()) {
            persist_encrypted_seed(seed_vec);
        }
        ...
        GetStrongRandBytes(pq_seed);
        seed_vec.assign(pq_seed.begin(), pq_seed.end());
        if (IsCrypted()) {
            persist_encrypted_seed(seed_vec);
        } else if (!batch.WritePQMasterSeed(seed_vec)) {
            throw std::runtime_error(std::string(__func__) + ": writing PQ master seed failed");
        }
    }

    const Span<const unsigned char> pq_seed{seed_vec};
    for (bool internal : {false, true}) {
        if (existing_p2mr[internal ? 1 : 0] != nullptr) {
            continue;
        }
        SetupPQDescriptorScriptPubKeyMan(batch, pq_seed, internal);
    }
}
...
void CWallet::SetupDescriptorScriptPubKeyMans()
{
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        if (!RunWithinTxn(GetDatabase(), /*process_desc=*/"setup pq descriptors", [&](WalletBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet){
            SetupOwnPQDescriptorScriptPubKeyMans(batch);
            return true;
        })) throw std::runtime_error("Error: cannot process db transaction for descriptors setup");
    } else {
        ExternalSigner signer = ExternalSignerScriptPubKeyMan::GetExternalSigner();

        // NOTE: add account parameter
        int account = 0;
...
util::Result<void> CWallet::ApplyMigrationData(WalletBatch& local_wallet_batch, MigrationData& data)
{
    ...
    // A watch-only legacy wallet must not gain a fresh spendable wallet during
    // migration. Only create replacement managers when key material existed.
    const bool has_spendable_material{!data.desc_spkms.empty() || data.master_key.key.IsValid()};
    ...
    // Setup new descriptors only when the legacy wallet had spendable material.
    ...
    if (has_spendable_material && !IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
        ...
        if (data.master_key.key.IsValid()) {
            ...
            // Setup with a new PQ-native seed if we don't have an existing master key.
            SetupOwnPQDescriptorScriptPubKeyMans(local_wallet_batch);
        }
    }

    // Get best block locator so that we can copy it to the watchonly and solvables.
    // The record did not exist in ancient wallets, so absence is valid.
    ...
    if (data.watchonly_wallet) {
        ...
        // An empty locator is valid and triggers a rescan on load.
...
// Used for backup files and derived watch-only/solvables wallet names.
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet> local_wallet, const SecureString& passphrase, WalletContext& context, bool was_loaded)
{
    ...
    // Make a backup of the DB in the wallet directory with a name derived
    // from the actual wallet path, including direct-file and unnamed wallets.
    fs::path this_wallet_dir = fs::absolute(fs::PathFromString(local_wallet->GetDatabase().Filename())).parent_path();
    const std::string backup_prefix = wallet_name.empty() ? MigrationPrefixName(*local_wallet) : [&] {
        const fs::path legacy_wallet_path{fs::weakly_canonical(GetWalletDir() / fs::PathFromString(wallet_name))};
    ...
    bool empty_local_wallet{false};
    {
        ...
        // Do the migration of keys and scripts only when legacy records exist.
        // Empty non-blank watch-only wallets need the same no-op conversion as
        // blank wallets, otherwise migration dereferences a missing legacy SPKM.
    ...
    // In case of loading failure, remember only files/directories created during migration.
    ...
    auto track_for_cleanup = [&](const CWallet& wallet) {
        const std::vector<fs::path> files{wallet.GetDatabase().Files()};
    ...
    if (success) {
        ...
        if (empty_local_wallet) {
            const std::vector<fs::path> empty_wallet_files{local_wallet->GetDatabase().Files()};
            const fs::path empty_wallet_dir{fs::PathFromString(local_wallet->GetDatabase().Filename()).parent_path()};
            local_wallet.reset();
            for (const fs::path& file : empty_wallet_files) fs::remove(file);
            if (fs::is_empty(empty_wallet_dir)) fs::remove(empty_wallet_dir);
        }

        // Reload the generated wallets in intentional priority order. For a
        // watch-only-only migration the watch-only wallet becomes the result.
    ...
    if (!success) {
        ...
        // Get files/directories to remove after unloading
        ...
        // Delete only files created by migration.
        for (const fs::path& file : wallet_files_to_remove) {
            fs::remove(file);
        }
        // Delete only newly created wallet directories and only if now empty.
        for (const fs::path& dir : wallet_empty_dirs_to_remove) {
            if (fs::is_empty(dir)) {
                fs::remove(dir);
            } else {
                LogWarning("Not removing non-empty wallet directory after failed migration: %s", fs::PathToString(dir));
            }
        }
...
{
    AssertLockHeld(cs_wallet);
    if (m_last_block_processed.IsNull()) return;

    CBlockLocator locator;
    chain().findBlock(m_last_block_processed, FoundBlock().locator(locator));
    if (!locator.IsNull()) {
        WalletBatch batch(GetDatabase());
        batch.WriteBestBlock(locator);
    }
}

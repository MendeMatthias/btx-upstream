...
BOOST_FIXTURE_TEST_CASE(chainstate_shallow_reorg_hysteresis_defers_until_work_margin, TestChain100Setup)
{
    ...
    // Heartbeat must keep last_deferred_unix fresh while the same episode
    // stays deferred, or mining-guard goes blind after
    // deferred_reorg_watch_seconds.
    const int64_t first_deferred_unix = stats.last_deferred_unix;
    const int64_t heartbeat_now = std::max<int64_t>(first_deferred_unix, 1) + 10;
    SetMockTime(heartbeat_now);
    BlockValidationState heartbeat_state;
    BOOST_CHECK(chainstate.ActivateBestChain(heartbeat_state));
    {
        LOCK(::cs_main);
        BOOST_CHECK_EQUAL(chainstate.m_chain.Tip()->GetBlockHash(), original_tip_hash);
        BOOST_CHECK_EQUAL(chainstate.setBlockIndexCandidates.count(competing_tip), 1);
    ...
    stats = ProbeReorgProtectionRuntimeStats();
    BOOST_CHECK_EQUAL(stats.deferred_reorgs, 1U);
    BOOST_CHECK_EQUAL(stats.last_deferred_unix, heartbeat_now);
    SetMockTime(0);

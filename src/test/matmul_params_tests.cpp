...
BOOST_AUTO_TEST_CASE(matmul_mainnet_prehash_fairness_upgrade_matches_asert_upgrade_window)
{
    ...
    BOOST_CHECK_EQUAL(main_consensus.nMatMulAsertHalfLifeUpgradeHeight, 191'715);
    BOOST_CHECK_EQUAL(main_consensus.nMatMulPowLimitUpgradeHeight, 191'714);
...
BOOST_AUTO_TEST_CASE(matmul_params_invariants)
{
    for (const auto chain_type : {ChainType::MAIN, ChainType::TESTNET, ChainType::REGTEST, ChainType::SHIELDEDV2DEV}) {
        ...
        if (c.nMatMulV4Height != std::numeric_limits<int32_t>::max()) {
            BOOST_CHECK_EQUAL(c.nMatMulV4TranscriptBlockSize, matmul::v4::kTileB);
        }

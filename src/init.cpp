...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-modelrpcsocket=<path>", "Unix socket for btx-modeld JSON-RPC (default: <datadir>/modelnet/modeld.sock). btxd never starts the helper; if the helper is down, model RPCs fail closed and the chain continues.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#endif
    ...
    argsman.AddArg("-miningchainguarddefaultmesh", "When local mining chain_guard is unhealthy, periodically enroll the configured mining peer mesh (-miningpeermesh, or the compiled defaults when unset) into the runtime addnode set so daemon-only mining nodes converge without an external supervisor (default: 1 on mainnet when -miningchainguard is enabled)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningpeermesh=<host:port>", "Peers the mining chain guard will manually connect to when the miner mesh degrades (repeatable). When set, replaces the compiled default mesh entirely so a fork can point this at its own hosts and operators can opt out of the bundled domains. Manual connections receive inbound-authority exemptions (IsManualConn / NoBan) that ordinary peers do not. Default when unset: node.btx.dev:19335, node.btxchain.org:19335, node.btx.tools:19335. Does not change DNS seeds or trusted-mode pin keys.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);

...
bool static IsDefinedSchnorrHashtype(const uint8_t hash_type, const bool allow_default)
{
    if (allow_default && hash_type == SIGHASH_DEFAULT) return true;
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    auto p2mr_stack_bytes_ok = [&stack, &altstack, sigversion]() {
        if (sigversion != SigVersion::P2MR) return true;

        uint64_t stack_bytes{0};
        for (const auto& elem : stack) {
            stack_bytes += elem.size();
            if (stack_bytes > MAX_P2MR_STACK_BYTES) return false;
        }
        for (const auto& elem : altstack) {
            stack_bytes += elem.size();
            if (stack_bytes > MAX_P2MR_STACK_BYTES) return false;
        }
        return true;
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            if (!script.GetOp(pc, opcode, vchPushValue))
                return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
            const size_t max_push_size = sigversion == SigVersion::P2MR ? MAX_P2MR_ELEMENT_SIZE : MAX_SCRIPT_ELEMENT_SIZE;
            if (vchPushValue.size() > max_push_size)
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKTEMPLATEVERIFY:
                {
                    ...
                    const valtype& ctv_hash = stacktop(-1);
                    if (ctv_hash.size() != 32) {
                        return set_error(serror, SCRIPT_ERR_CTV_HASH_SIZE);
                    }
                    if (!checker.CheckCTVHash(ctv_hash)) {
                        return set_error(serror, SCRIPT_ERR_CTV_HASH_MISMATCH);
                    }
                }
                break;
                ...
                case OP_NOTIF:
                {
                    ...
                    if (fExec)
                    {
                        ...
                        // Tapscript and P2MR require minimal IF/NOTIF inputs as
                        // consensus rules.
                ...
                case OP_CHECKSIG_SLHDSA:
                {
                    ...
                    const valtype& pubkey = stacktop(-1);
                    PQAlgorithm algo;
                    bool is_checksigadd{false};
                    if (!DecodeP2MRChecksigOpcode(opcode, algo, is_checksigadd) || is_checksigadd) {
                        return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
                    }
                    if (algo == PQAlgorithm::ML_DSA_44 && (flags & SCRIPT_VERIFY_DISALLOW_MLDSA)) {
                        return set_error(serror, SCRIPT_ERR_DISABLED_OPCODE);
                    }
                    const size_t expected_sig_size = GetPQSignatureSize(algo);
                    const size_t expected_pubkey_size = GetPQPubKeySize(algo);
                    const ScriptError sig_err = algo == PQAlgorithm::ML_DSA_44 ? SCRIPT_ERR_SIG_MLDSA : SCRIPT_ERR_SIG_SLHDSA;

                    if (pubkey.size() != expected_pubkey_size) {
                        return set_error(serror, SCRIPT_ERR_PQ_PUBKEY_SIZE);
                    }

                    uint8_t hash_type = SIGHASH_DEFAULT;
                    if (!sig.empty() && sig.size() == expected_sig_size + 1) {
                        hash_type = sig.back();
                        sig.pop_back();
                        if (!IsDefinedSchnorrHashtype(hash_type, /*allow_default=*/false)) {
                            return set_error(serror, SCRIPT_ERR_SIG_HASHTYPE);
                        }
                    } else if (!sig.empty() && sig.size() != expected_sig_size) {
                    ...
                    if (!sig.empty()) {
                        assert(execdata.m_validation_weight_left_init);
                        const int64_t weight_cost = algo == PQAlgorithm::ML_DSA_44 ? VALIDATION_WEIGHT_PER_MLDSA_SIGOP : VALIDATION_WEIGHT_PER_SLHDSA_SIGOP;
                ...
                case OP_CHECKSIGADD_SLHDSA:
                {
                    ...
                    if (!sig.empty()) {
                        assert(execdata.m_validation_weight_left_init);
                        const int64_t weight_cost = algo == PQAlgorithm::ML_DSA_44 ? VALIDATION_WEIGHT_PER_MLDSA_MULTISIG_SIGOP : VALIDATION_WEIGHT_PER_SLHDSA_MULTISIG_SIGOP;
                        execdata.m_validation_weight_left -= weight_cost;
                        if (execdata.m_validation_weight_left < 0) {
                            return set_error(serror, SCRIPT_ERR_TAPSCRIPT_VALIDATION_WEIGHT);
                        }
                    ...
                    popstack(stack);
                    popstack(stack);
                    stack.push_back((n + (success ? 1 : 0)).getvch());
                }
                break;
                ...
                case OP_CHECKSIGFROMSTACK:
                {
                    ...
                    const auto algo = GetPQAlgorithmByPubKeySize(pubkey.size());
                    if (!algo.has_value()) {
                        return set_error(serror, SCRIPT_ERR_PQ_PUBKEY_SIZE);
                    }
                    if (*algo == PQAlgorithm::ML_DSA_44) {
                        if (flags & SCRIPT_VERIFY_DISALLOW_MLDSA) {
                            return set_error(serror, SCRIPT_ERR_DISABLED_OPCODE);
                        }
                    }
...
void PrecomputedTransactionData::Init(const T& txTo, std::vector<CTxOut>&& spent_outputs, bool force)
{
    ...
    for (size_t inpos = 0; inpos < txTo.vin.size() && !(uses_bip143_segwit && uses_bip341_taproot); ++inpos) {
        if (!txTo.vin[inpos].scriptWitness.IsNull()) {
            if (m_spent_outputs_ready && m_spent_outputs[inpos].scriptPubKey.size() == 2 + WITNESS_V1_TAPROOT_SIZE &&
                ...
                // Treat every witness-bearing spend with 34-byte scriptPubKey that starts with OP_1 (Taproot) or
                // OP_2 (P2MR) as a BIP341-style spend.
...

    {
        ...
        for (const auto& txin : txTo.vin) {
            ss << txin.scriptSig;
            has_scriptsigs |= !txin.scriptSig.empty();
        }
        m_ctv_scriptsigs_hash = ss.GetSHA256();
        m_ctv_has_scriptsigs = has_scriptsigs;

        m_ctv_has_shielded_bundle = txTo.HasShieldedBundle();
        if (m_ctv_has_shielded_bundle) {
            const CShieldedBundle& bundle = txTo.GetShieldedBundle();
            m_ctv_shielded_bundle_hash = ComputeShieldedBundleCtvHash(bundle);
        } else {
            m_ctv_shielded_bundle_hash.SetNull();
        }
        m_ctv_ready = true;
    }
}
...
uint256 ComputeCTVHashImpl(const T& tx, uint32_t nIn, const PrecomputedTransactionData& txdata)
{
    ...
    if (txdata.m_ctv_has_shielded_bundle) {
        ss << txdata.m_ctv_shielded_bundle_hash;
    }
    ss << nIn;
    return ss.GetSHA256();
}
...
{
    return ComputeCTVHashImpl(tx, nIn, txdata);
}
...
bool SignatureHashSchnorr(uint256& hash_out, ScriptExecutionData& execdata, const T& tx_to, uint32_t in_pos, uint8_t hash_type, SigVersion sigversion, const PrecomputedTransactionData& cache, MissingDataBehavior mdb)
{
    ...
    // Epoch (Taproot/Tapscript use 0, P2MR uses 2).
    const uint8_t epoch = sigversion == SigVersion::P2MR ? uint8_t{2} : uint8_t{0};
    ss << epoch;
    ...
    // Additional data for BIP 342-like script-path signatures.
    if (sigversion == SigVersion::TAPSCRIPT || sigversion == SigVersion::P2MR) {
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckCTVHash(Span<const unsigned char> hash_from_stack) const
{
    assert(hash_from_stack.size() == uint256::size());
    if (!this->txdata) return HandleMissingData(m_mdb);

    const uint256 computed = ComputeCTVHash(*txTo, nIn, *this->txdata);
    return std::memcmp(hash_from_stack.data(), computed.begin(), uint256::size()) == 0;
}
...
static bool ExecuteWitnessScript(const Span<const valtype>& stack_span, const CScript& exec_script, unsigned int flags, SigVersion sigversion, const BaseSignatureChecker& checker, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    if (sigversion == SigVersion::TAPSCRIPT || sigversion == SigVersion::P2MR) {
        ...
        while (pc < exec_script.end()) {
            ...
            const bool is_op_success = sigversion == SigVersion::TAPSCRIPT
                ? IsOpSuccess(opcode)
                : IsOpSuccessP2MR(opcode);
            if (is_op_success) {
    ...
    // Disallow oversized stack items in witness stack.
    uint64_t witness_stack_bytes{0};
    for (const valtype& elem : stack) {
        const size_t max_elem_size = sigversion == SigVersion::P2MR ? MAX_P2MR_ELEMENT_SIZE : MAX_SCRIPT_ELEMENT_SIZE;
        if (elem.size() > max_elem_size) {
            return set_error(serror, SCRIPT_ERR_PUSH_SIZE);
        }
        witness_stack_bytes += elem.size();
        if (sigversion == SigVersion::P2MR && witness_stack_bytes > MAX_P2MR_STACK_BYTES) {
            return set_error(serror, SCRIPT_ERR_STACK_SIZE);
        }
    }
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        // P2MR: 32-byte witness v2 program committing to a post-quantum script Merkle root.
        if (stack.size() < 2) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_WITNESS_EMPTY);
        }
...
size_t static WitnessSigOps(int witversion, const std::vector<unsigned char>& witprogram, const CScriptWitness& witness)
{
    ...
    if (witversion == 2 && witprogram.size() == WITNESS_V2_P2MR_SIZE && witness.stack.size() >= 2) {
        ...
        while (pc < leaf_script.end()) {
            opcodetype opcode;
            if (!leaf_script.GetOp(pc, opcode)) break;
            switch (opcode) {
            case OP_CHECKSIG_MLDSA:

# `btx.conf` Configuration File

The configuration file is used by `btxd`, `btx-qt` and `btx-cli`.

All command-line options (except for `-?`, `-help`, `-version` and `-conf`) may be specified in a configuration file, and all configuration file options (except for `includeconf`) may also be specified on the command line. Command-line options override values set in the configuration file and configuration file options override values set in the GUI.

Changes to the configuration file while `btxd` or `btx-qt` is running only take effect after restarting.
...
The `settings.json` file contains dynamic settings that are set by the BTX GUI and RPCs at runtime, and augment or replace the static settings specified in the `btx.conf` file.

Command line options also augment or replace `btx.conf` options, and can be useful for scripting and debugging.

It is possible to see which setting values are in use by checking `debug.log` output. Any unrecognized options that are found in `btx.conf` also show up as warnings in `debug.log` output.
...
The configuration file is not automatically created; you can create it using your favorite text editor. By default, the configuration file name is `btx.conf` and it is located in the BTX data directory, but both the data directory and the configuration file path may be changed using the `-datadir` and `-conf` command-line options.

The `includeconf=<file>` option in the `btx.conf` file can be used to include additional configuration files.

### Default configuration file locations

Operating System | Data Directory | Example Path
-- | -- | --
Windows | `%LOCALAPPDATA%\BTX\` | `C:\Users\username\AppData\Local\BTX\btx.conf`
Linux | `$HOME/.btx/` | `/home/username/.btx/btx.conf`
macOS | `$HOME/Library/Application Support/BTX/` | `/Users/username/Library/Application Support/BTX/btx.conf`

An example configuration file can be generated by [contrib/devtools/gen-btx-node-conf.sh](../contrib/devtools/gen-btx-node-conf.sh).
Run this script after compiling to generate an up-to-date configuration file.
The output is placed under `share/examples/btx.conf`.

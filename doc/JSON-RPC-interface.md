...
This is the endpoint used by `btx-cli` (legacy alias: `bitcoin-cli`) when a
`-rpcwallet=` parameter is passed in.
...
```sh
# Get block count from the / endpoint when rpcuser=alice and rpcport=19334
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "getblockcount", "params": []}' -H 'content-type: application/json' localhost:19334/

# Get shielded balance from the /wallet/walletname endpoint when rpcuser=alice, rpcport=19334 and rpcwallet=desc-wallet
# The result object includes spendable `balance`, `note_count`, and optional watch-only fields.
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "z_getbalance", "params": []}' -H 'content-type: application/json' localhost:19334/wallet/desc-wallet

```

## Bridge wallet RPCs

The wallet endpoint also exposes the BTX-side bridge settlement interface used
by external L2 / bridge operators:
...
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "bridge_planout", "params": ["<operator_pq_pubkey>", "<refund_pq_pubkey>", "btx1...", 4, {"bridge_id":"<32-byte hex>","operation_id":"<32-byte hex>","refund_lock_height":1440}]}' -H 'content-type: application/json' localhost:19334/wallet/desc-wallet
```

## BTX operator quick pointers

Fast-start service nodes usually interact with the `/` endpoint through these
MatMul admission-control RPCs:
...
The profile-based flow exposes the built-in `easy`, `normal`, `hard`, and
`idle` tiers together with average-node solve-time / throughput estimates so
operators can tune user-visible challenge budgets against current network
conditions. The profile RPCs also accept `solver_parallelism` and
`solver_duty_cycle_pct` so agents can convert those average-node estimates into
an explicit `operator_capacity` plan for idle-time mining or shared-gateway
sizing. `getmatmulservicechallengeplan` adds the inverse path: tell the node
the solves/hour, solves/day, or mean-seconds-between-solves you want under a
given worker budget, and it returns a direct `issue_defaults` plan plus the
closest profile matches. The profile and planner surfaces now expose
`issue_defaults` / `profile_issue_defaults` so clients can round-trip directly
into issuance RPCs without reconstructing parameters by hand. For the
end-to-end binary install and release-bundle workflow, see
[btx-download-and-go.md](btx-download-and-go.md) and
[../contrib/faststart/README.md](../contrib/faststart/README.md).

Useful operational additions in the current BTX RPC surface:

- `solvematmulservicechallenge` accepts optional `time_budget_ms` and
  `solver_threads` arguments so agentic clients can bound local solve work.
- `verifymatmulserviceproof` and `verifymatmulserviceproofs` accept an optional
  final boolean to skip local/shared registry lookups for stateless
  high-volume verification.
- `getdifficultyhealth` now includes a `service_challenge_registry` object that
...
btx-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
btx-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
btx-cli -named createwallet mywallet load_on_startup=true
```

## Versioning

The RPC interface might change from one major version of BTX to the next. This
makes the RPC interface implicitly versioned on the major version.
...
The RPC interface allows other programs to control BTX,
including the ability to spend funds from your wallets, affect consensus
verification, read private data, and otherwise perform operations that
can cause loss of money, data, or privacy.  This section suggests how
you should use and configure BTX to reduce the risk that its
RPC interface will be abused.

- **Securing the executable:** Anyone with physical or remote access to
  the computer, container, or virtual machine running BTX can
  compromise either the whole program or just the RPC interface.  This
  includes being able to record any passphrases you enter for unlocking
  your encrypted wallets or changing settings so that your BTX
  program tells you that certain transactions have multiple
  confirmations even when they aren't part of the best block chain.  For
  this reason, you should not use BTX for security sensitive
...
- **Securing local network access:** By default, the RPC interface can
  ...
  an RPC interface on the same port as used by BTX in order to
  trick you into revealing your authentication credentials.  For this
  reason, it is important to only use BTX for
  security-sensitive operations on a computer whose other programs you
  trust.

- **Securing remote network access:** You may optionally allow other
  computers to remotely control BTX by setting the `rpcallowip`
  ...
  Internet.**  Although BTX's RPC interface does use
  ...
  `btxd -help` for more information about these settings and other
  settings described in this document.

    Related, if you use BTX inside a Docker container, you may
    need to expose the RPC port to the host system.  The default way to
    do this in Docker also exposes the port to the public Internet.
    Instead, expose it only on the host system's localhost, for example:
    `-p 127.0.0.1:19334:19334`
    (if you intentionally keep the legacy RPC default, use `8332`).

- **Secure authentication:** By default, when no `rpcpassword` is specified, BTX generates unique
  login credentials each time it restarts and puts them into a file
  readable only by the user that started Bitcoin Core, allowing any of
  that user's RPC clients with read access to the file to login
  automatically.  The file is `.cookie` in the BTX
  configuration directory, and using these credentials is the preferred
  RPC authentication method.  If you need to generate static login
  credentials for your programs, you can use the script in the
  `share/rpcauth` directory in the BTX source tree.  As a final
